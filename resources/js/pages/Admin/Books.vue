  ...
  <div class="min-h-screen bg-[#e8e8e8]">
    ...
    <div class="flex min-h-[calc(100vh-60px)]">
      ...
      <main class="flex-1 px-[30px] py-[30px] bg-[#f5f5f5]">
        ...
        <div class="flex flex-col md:flex-row items-stretch md:items-center
          ...
          <!-- Add New Book -->
          <button type="button" @click="openAddBook"
      ...
      <div @click.stop class="bg-white rounded-[20px] p-[40px] w-full max-w-[900px]
        ...
        <div v-if="selectedBook">
          ...
          <div class="flex flex-col sm:flex-row gap-[12px] mt-[25px]">
            ...
            <button type="button" @click="deleteBook(selectedBook)"
              class="flex-1 px-[20px] py-[14px] rounded-[8px] border-0 text-[15px] font-semibold
    ...
    <div v-if="addBookOpen" @click.self="addBookOpen = false"
      ...
      <div @click.stop class="bg-white rounded-[20px] p-[40px] w-full max-w-[700px]
        ...
        <!-- Inline form errors -->
        <div v-if="Object.keys(addForm.errors).length"
             class="mb-4 bg-red-100 text-red-800 text-sm px-4 py-3 rounded-lg">
          <p v-for="(msg, key) in addForm.errors" :key="key">{{ msg }}</p>
        </div>
        ...
        <div class="flex flex-col gap-[20px]">
          ...
          <div class="flex flex-col gap-[8px]">
            ...
            <div class="flex flex-col items-center gap-[15px]">
              ...
              <input type="file" accept="image/*" class="hidden" ref="coverInput"
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-[20px]">
            <div class="flex flex-col gap-[8px]">
              ...
              <input type="text" v-model="addForm.isbn"
            ...
            <div class="flex flex-col gap-[8px]">
              ...
              <input type="text" v-model="addForm.title" required
                     class="px-[12px] py-[12px] border-2 border-[#a5d6a7] rounded-[8px]
                            text-[15px] outline-none focus:border-[#66bb6a]" />
            </div>
          </div>
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-[20px]">
            <div class="flex flex-col gap-[8px]">
              ...
              <input type="text" v-model="addForm.author"
                     class="px-[12px] py-[12px] border-2 border-[#a5d6a7] rounded-[8px]
                            text-[15px] outline-none focus:border-[#66bb6a]" />
            </div>
            <div class="flex flex-col gap-[8px]">
              <label class="font-semibold text-[14px] text-[#333]">Publisher:</label>
              <input type="text" v-model="addForm.publisher"
                     class="px-[12px] py-[12px] border-2 border-[#a5d6a7] rounded-[8px]
                            text-[15px] outline-none focus:border-[#66bb6a]" />
            </div>
          </div>
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-[20px]">
            <div class="flex flex-col gap-[8px]">
              ...
              <input type="text" v-model="addForm.year_published"
                     class="px-[12px] py-[12px] border-2 border-[#a5d6a7] rounded-[8px]
                            text-[15px] outline-none focus:border-[#66bb6a]" />
            </div>
            <div class="flex flex-col gap-[8px]">
              ...
              <input type="text" v-model="addForm.new_category" placeholder="e.g. Web Development"
                     class="px-[12px] py-[12px] border-2 border-[#a5d6a7] rounded-[8px]
                            text-[15px] outline-none focus:border-[#66bb6a]" />
            </div>
          </div>
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-[20px]">
            <div class="flex flex-col gap-[8px]">
              ...
              <input type="number" v-model="addForm.total_pages"
                     class="px-[12px] py-[12px] border-2 border-[#a5d6a7] rounded-[8px]
                            text-[15px] outline-none focus:border-[#66bb6a]" />
            </div>
            <div class="flex flex-col gap-[8px]">
          ...
          <div class="flex flex-col gap-[8px]">
            ...
            <textarea v-model="addForm.table_of_contents" rows="4" placeholder="Chapter 1 - Introduction"
                      class="px-[12px] py-[12px] border-2 border-[#a5d6a7] rounded-[8px]
                             text-[14px] resize-y outline-none focus:border-[#66bb6a]"></textarea>
          </div>
          ...
          <div class="flex flex-col sm:flex-row gap-[12px] mt-[25px]">
            ...
            <button type="button" @click="submitAddBook" :disabled="addForm.processing"
              class="flex-1 px-[20px] py-[14px] rounded-[8px] border-0 text-[15px] font-semibold
                     bg-[#2e7d32] text-white cursor-pointer transition-colors hover:bg-[#1b5e20]
                     disabled:opacity-60">
              {{ addForm.processing ? 'Saving...' : 'Save Book' }}
            </button>
          </div>
        </div>
      </div>
...
// ✅ Inertia form — handles CSRF automatically
...
const openAddBook = () => {
  ...
  addBookOpen.value = true
}

const handleCoverChange = (event) => {
  const file = event.target.files[0]
  if (!file) { coverPreview.value = null; addForm.cover = null; return }
  addForm.cover = file
...
// ✅ Submit using Inertia router — no manual CSRF needed
const submitAddBook = () => {
  addForm.post(route('admin.books.store'), {
    forceFormData: true, // needed because we have a file upload
    onSuccess: () => {
      addBookOpen.value = false
      addForm.reset()
      coverPreview.value = null
    },
  })
...
// ✅ Delete using Inertia router — no manual CSRF needed
const deleteBook = (book) => {
  if (!confirm('Delete this book? This cannot be undone.')) return
  router.delete(route('admin.books.destroy', book.id), {
    onSuccess: () => { bookViewOpen.value = false },
  })
}
