  ...
  <div class="min-h-screen bg-[#e8e8e8]">
    ...
    <div v-if="requestModalOpen" @click.self="requestModalOpen = false"
      ...
      <div @click.stop class="bg-white rounded-[20px] p-[40px] w-full max-w-[700px]
        ...
        <div v-if="requestSettingsForm.errors && Object.keys(requestSettingsForm.errors).length"
             class="mb-4 bg-red-100 text-red-700 text-sm px-4 py-3 rounded-lg">
          <p v-for="(msg, key) in requestSettingsForm.errors" :key="key">{{ msg }}</p>
        </div>
        ...
        <div class="flex flex-col gap-[20px]">
          <div class="flex flex-col gap-[8px]">
            ...
            <select v-model="requestSettingsForm.max_chapters_per_request"
                    class="px-[12px] py-[12px] border-2 border-[#e0e0e0] rounded-[8px] text-[15px] outline-none focus:border-[#66bb6a]">
              <option v-for="n in [1,2,3,5,10]" :key="n" :value="n">{{ n }} Chapter{{ n > 1 ? 's' : '' }}</option>
            </select>
          </div>
          ...
          <div class="flex flex-col gap-[8px]">
            ...
            <input type="number" v-model="requestSettingsForm.request_expiry_days" min="1" max="30"
          ...
          <div class="flex flex-col gap-[8px]">
            ...
            <div class="flex flex-col gap-[12px] p-[15px] bg-[#f9f9f9] rounded-[8px]">
              <label class="flex items-center gap-[10px] text-[14px] cursor-pointer">
                <input type="checkbox" v-model="requestSettingsForm.notify_on_request"
                       class="w-[18px] h-[18px] cursor-pointer accent-[#4caf50]" />
                <span class="text-[#555]">Notify admin on new request</span>
              </label>
              <label class="flex items-center gap-[10px] text-[14px] cursor-pointer">
                <input type="checkbox" v-model="requestSettingsForm.notify_on_complete"
                       class="w-[18px] h-[18px] cursor-pointer accent-[#4caf50]" />
                <span class="text-[#555]">Notify student on completion</span>
              </label>
              <label class="flex items-center gap-[10px] text-[14px] cursor-pointer">
                <input type="checkbox" v-model="requestSettingsForm.notify_on_expiry"
                       class="w-[18px] h-[18px] cursor-pointer accent-[#4caf50]" />
          ...
          <div class="flex flex-col sm:flex-row gap-[12px] mt-[25px]">
            ...
            <button type="button" @click="submitRequestSettings"
              :disabled="requestSettingsForm.processing"
              class="flex-1 px-[20px] py-[14px] rounded-[8px] border-0 text-[15px] font-semibold
                     bg-[#2e7d32] text-white cursor-pointer transition-colors hover:bg-[#1b5e20] disabled:opacity-60">
              {{ requestSettingsForm.processing ? 'Saving...' : 'Save Settings' }}
            </button>
          </div>
        </div>
      </div>
      ...
      <div @click.stop class="bg-white rounded-[20px] p-[40px] w-full max-w-[700px]
        ...
        <div class="flex flex-col gap-[20px]">
          <div class="flex flex-col gap-[8px]">
            ...
            <input type="text" v-model="libraryForm.library_name" placeholder="Enter library name"
                   class="px-[12px] py-[12px] border-2 border-[#e0e0e0] rounded-[8px] text-[15px] outline-none focus:border-[#66bb6a]" />
          </div>
          <div class="flex flex-col gap-[8px]">
            <label class="font-semibold text-[15px] text-[#333] flex items-center gap-[8px]">🕐 Operation Hours:</label>
            <input type="text" v-model="libraryForm.operation_hours" placeholder="e.g., Mon-Fri: 8:00 AM - 5:00 PM"
                   class="px-[12px] py-[12px] border-2 border-[#e0e0e0] rounded-[8px] text-[15px] outline-none focus:border-[#66bb6a]" />
          </div>
          <div class="flex flex-col gap-[8px]">
            <label class="font-semibold text-[15px] text-[#333] flex items-center gap-[8px]">📞 Contact Number:</label>
            <input type="tel" v-model="libraryForm.library_contact" placeholder="Enter contact number"
                   class="px-[12px] py-[12px] border-2 border-[#e0e0e0] rounded-[8px] text-[15px] outline-none focus:border-[#66bb6a]" />
          </div>
          <div class="flex flex-col gap-[8px]">
            <label class="font-semibold text-[15px] text-[#333] flex items-center gap-[8px]">📧 Email Address:</label>
            <input type="email" v-model="libraryForm.library_email" placeholder="Enter email address"
                   class="px-[12px] py-[12px] border-2 border-[#e0e0e0] rounded-[8px] text-[15px] outline-none focus:border-[#66bb6a]" />
          </div>
          <div class="flex flex-col gap-[8px]">
            <label class="font-semibold text-[15px] text-[#333] flex items-center gap-[8px]">📍 Library Address:</label>
            <textarea v-model="libraryForm.library_address" rows="3" placeholder="Enter complete library address"
                      class="px-[12px] py-[12px] border-2 border-[#e0e0e0] rounded-[8px] text-[15px] resize-y outline-none focus:border-[#66bb6a]"></textarea>
          </div>
          <div class="flex flex-col sm:flex-row gap-[12px] mt-[25px]">
            <button type="button" @click="libraryModalOpen = false"
              class="flex-1 px-[20px] py-[14px] rounded-[8px] border-0 text-[15px] font-semibold
                     bg-[#a5d6a7] text-black cursor-pointer transition-colors hover:bg-[#81c784]">Cancel</button>
            <button type="button" @click="submitLibrary"
              :disabled="libraryForm.processing"
              class="flex-1 px-[20px] py-[14px] rounded-[8px] border-0 text-[15px] font-semibold
                     bg-[#2e7d32] text-white cursor-pointer transition-colors hover:bg-[#1b5e20] disabled:opacity-60">
              {{ libraryForm.processing ? 'Saving...' : 'Save Information' }}
            </button>
          </div>
        </div>
      </div>
        ...
        <div class="mb-[30px]">
          ...
          <div class="flex flex-col gap-[8px] mb-[15px]">
            ...
            <input type="file" accept="image/*"
                   @change="e => accountForm.avatar = e.target.files[0]"
                   class="px-[12px] py-[12px] border-2 border-[#e0e0e0] rounded-[8px] text-[15px] outline-none" />
          </div>
          ...
          <div class="flex flex-col sm:flex-row gap-[12px] mt-[25px]">
            <button type="button" @click="accountModalOpen = false"
              class="flex-1 px-[20px] py-[14px] rounded-[8px] border-0 text-[15px] font-semibold
                     bg-[#a5d6a7] text-black cursor-pointer transition-colors hover:bg-[#81c784]">Cancel</button>
            <button type="button" @click="submitAccount"
        ...
        <div class="pt-[30px] border-t-2 border-[#e0e0e0]">
          ...
          <div class="flex flex-col gap-[15px]">
            <div class="flex flex-col gap-[8px]">
              ...
              <input type="password" v-model="adminPasswordForm.current_password"
                     class="px-[12px] py-[12px] border-2 border-[#e0e0e0] rounded-[8px] text-[15px] outline-none focus:border-[#66bb6a]" />
            </div>
            <div class="flex flex-col gap-[8px]">
              <label class="font-semibold text-[14px] text-[#333]">New Password:</label>
              <input type="password" v-model="adminPasswordForm.password" minlength="8"
                     class="px-[12px] py-[12px] border-2 border-[#e0e0e0] rounded-[8px] text-[15px] outline-none focus:border-[#66bb6a]" />
            </div>
            <div class="flex flex-col gap-[8px]">
              <label class="font-semibold text-[14px] text-[#333]">Confirm Password:</label>
              <input type="password" v-model="adminPasswordForm.password_confirmation"
                     class="px-[12px] py-[12px] border-2 border-[#e0e0e0] rounded-[8px] text-[15px] outline-none focus:border-[#66bb6a]" />
            </div>
            <button type="button" @click="submitAdminPassword"
              :disabled="adminPasswordForm.processing"
              class="w-full px-[20px] py-[14px] rounded-[8px] border-0 text-[15px] font-semibold
                     bg-[#2e7d32] text-white cursor-pointer mt-[15px] transition-colors hover:bg-[#1b5e20] disabled:opacity-60">
              {{ adminPasswordForm.processing ? 'Changing...' : 'Change Password' }}
            </button>
          </div>
        </div>
      </div>
...
const requestSettingsForm = useForm({
  max_chapters_per_request: props.settings.max_chapters_per_request ?? 1,
  request_expiry_days:      props.settings.request_expiry_days      ?? 7,
  notify_on_request:        props.settings.notify_on_request        ?? false,
  notify_on_complete:       props.settings.notify_on_complete       ?? false,
  notify_on_expiry:         props.settings.notify_on_expiry         ?? false,
})
const submitRequestSettings = () => {
  requestSettingsForm.post(route('admin.settings.update'), {
    onSuccess: () => { requestModalOpen.value = false },
  })
}
...
const libraryForm = useForm({
  ...
  library_contact: props.settings.library_contact ?? '',
  library_email:   props.settings.library_email   ?? '',
  library_address: props.settings.library_address ?? '',
})
const submitLibrary = () => {
  libraryForm.post(route('admin.settings.update'), {
    onSuccess: () => { libraryModalOpen.value = false },
  })
}
...
const submitAccount = () => {
  accountForm.post(route('admin.account.update'), {
    forceFormData: true,
    onSuccess: () => { accountModalOpen.value = false },
  })
}
...
const submitAdminPassword = () => {
  adminPasswordForm.post(route('admin.account.password'), {
    onSuccess: () => {
