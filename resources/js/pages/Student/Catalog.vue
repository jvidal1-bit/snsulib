  ...
  <div class="min-h-screen bg-gray-100 pb-20">
    ...
    >
      <div class="bg-white rounded-2xl shadow-xl max-w-md w-full mx-4 p-6">
        ...
        <!-- Inline form errors -->
        <div v-if="Object.keys(requestForm.errors).length"
             class="mb-4 bg-red-100 text-red-800 text-sm px-4 py-3 rounded-lg">
          <p v-for="(msg, key) in requestForm.errors" :key="key">{{ msg }}</p>
        </div>
          ...
          <div class="flex gap-3 mt-4">
            ...
            <button
              ...
              class="flex-1 py-3 px-4 rounded-full text-sm font-semibold
                     ...
                     disabled:opacity-60"
            >{{ requestForm.processing ? 'Submitting...' : 'Submit Request' }}</button>
          </div>
        </div>
      </div>
    </div>
...
// ✅ Inertia form — handles CSRF automatically on every submit
...
const openRequest = () => {
  ...
  // Populate the form with the selected book id
  requestForm.book_id = selectedBook.value.id
  requestForm.chapter = ''
  requestForm.purpose = ''
  requestForm.needed_by = ''
  requestForm.note = ''
...
// ✅ Submit via Inertia — CSRF token is handled automatically, no 419 ever
const submitRequest = () => {
  requestForm.post(route('student.requests.store'), {
    onSuccess: () => {
      closeRequest()
      closeBook()
    },
