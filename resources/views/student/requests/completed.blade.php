...
@section('title', 'Completed - SNSU Library E-Request')
...
@section('content')
    <div class="min-h-screen bg-gray-100" x-data="completedPage()">
        {{-- Navbar --}}
                        ...
                        <tbody>
                            @forelse($requests as $requestItem)
                                ...
                                <tr class="border-b border-gray-200 hover:bg-gray-100 transition">
                                    ...
                                    <td class="px-4 py-3">
                                        {{-- completed badge style from CSS --}}
                                        <span class="inline-flex px-4 py-1 rounded-full text-xs font-semibold bg-[#d1e7dd] text-[#0f5132]">
                                            Completed
                                        </span>
                                    </td>
                                    <td class="px-4 py-3 text-gray-700">
                                        @if($requestItem->expiration_at)
                                            <span class="{{ $expired ? 'text-red-600 font-semibold' : '' }}">
                                                {{ $requestItem->expiration_at->format('m/d/y H:i') }}
                                                @if($expired)
                                                    (Expired)
                                                @endif
                                            </span>
                                    ...
                                    <td class="px-4 py-3 text-center">
                                        <div class="flex items-center justify-center gap-2">
                                            ...
                                            @if($requestItem->completed_file && !$expired)
                                                <a href="{{ asset('storage/' . $requestItem->completed_file) }}"
                                                target="_blank"
